...
@section('content')
    <div>
        <form method="post" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col gap-4 m-4" action=" {{ route('employee.update',['employee'=>$employee]) }} ">
            @method('PUT')
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <div class="">
                    <select id="company" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('company') is-invalid @enderror" name="company_id" value="{{ $employee->company->id }}" required>
                        @foreach($companies as $c)
                            <option {{ ($c->id === $employee->company->id)?"selected":"" }} value="{{ $c->id }}">{{ $c->name }}</option>
                        @endforeach
                    </select>
